...
class AbstractPlotter(Object):
    ...
    @abc.abstractmethod
    def _plot_traces(self, emulators, planners, measurements, plot_file_name, *args, **kwargs):
        pass

    @abc.abstractmethod
    def _plot_traces_regret(self, emulators, planners, measurements, plot_file_name, *args, **kwargs):
        pass

    @abc.abstractmethod
    def _plot_traces_rank(self, emulators, planners, measurements, plot_file_name, *args, **kwargs):
        pass

    @abc.abstractmethod
    def _plot_traces_fraction_top_k(self, emulators, planners, measurements, threshold, is_percent, plot_file_name,*args, **kwargs):
        pass

    @abc.abstractmethod
    def _plot_num_evals_top_k(self, emulators, planners, measurements, threshold, is_percent, plot_file_name,*args, **kwargs):
        pass
    ...
    def plot_from_db(
        ...
        **kwargs,
        ...
        if kind == 'traces':
            ...
            self._plot_traces(emulators, planners, measurements, plot_file_name, *args, **kwargs)
        elif kind == 'traces_regret':
            emulators, planners, measurements = self._get_traces_regret(campaigns)
            self._plot_traces_regret(emulators, planners, measurements, plot_file_name, *args, **kwargs)
        elif kind == 'traces_rank':
            emulators, planners, measurements = self._get_traces_rank(campaigns)
            self._plot_traces_rank(emulators, planners, measurements, plot_file_name, *args, **kwargs)
        elif kind == 'traces_fraction_top_k':
            emulators, planners, measurements = self._get_traces_fraction_top_k(campaigns, threshold, is_percent )
            self._plot_traces_fraction_top_k(emulators, planners, measurements, threshold, is_percent, plot_file_name, *args, **kwargs)
        ...
        for campaign in campaigns:
            ...
            if emulator_type == 'numeric':
                ...
                surf = Surface(
                    kind=campaign.surface_kind, 
                    param_dim=len(campaign.param_space),
                    num_opts=len(campaign.param_space[0]['options']), # num_opts will be the same along each dimension
                )
                values = np.array(
            ...
            for val_index, value in enumerate(campaign.best_values):
                measurements[camp_emulator][planner]['idxs'].append(val_index)
                measurements[camp_emulator][planner]['vals'].append( ranks[value[0]] ) # rank
        ...
        for campaign in campaigns:
            ...
            if emulator_type == 'numeric':
                ...
                if not is_percent:
                    ...
                    percent_val = (threshold/100) * (np.amax(sort_values)-np.amin(sort_values)) + np.amin(sort_values)
                    if campaign.goal == 'minimize':
                        sort_values = sort_values[:np.amin(np.where(sort_values>percent_val))]
                    if campaign.goal == 'maximize':
                        sort_values = sort_values[:np.amin(np.where(sort_value<percent_val))]
                ...
                values = np.array(surf.run(self._create_available_options(surf.param_space) )).squeeze()
                sort_idxs = np.argsort(values, axis=0)
                if campaign.goal == 'minimize':
                    pass 
                elif campaign.goal == 'maximize':
            ...
            for val_index in range(len(campaign_values)):
                ...
                measurements[camp_emulator][planner]['vals'].append(
                        sum(val in campaign_values[:val_index] for val in sort_values)/len(sort_values)
    ...
    def _validate_plot_kind(self, problem_type, kind):
        ...
        supported_kinds = [method.split('_plot_')[1] for method in dir(self) if method.startswith('_plot_')]
