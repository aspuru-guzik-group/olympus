...
from . import AbstractModel, get_models_list, import_model
...
def _validate_model_kind(kind):
    ...
    if type(kind) == str:
        ...
        if kind not in avail_models:
            message = (
                'Model "{0}" not available in Olympus. Please choose '
                "from one of the available models: {1}".format(
                    kind, ", ".join(avail_models)
                )
            )
