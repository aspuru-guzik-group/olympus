...
from . import AbstractNoise, get_noises_list, import_noise
...
def _validate_noise_kind(kind):
    ...
    if type(kind) == str:
        ...
        if kind not in avail_noises:
            message = (
                'Noise "{0}" not available in Olympus. Please choose '
                "from one of the available noise objects: {1}".format(
                    kind, ", ".join(avail_noises)
                )
            )
